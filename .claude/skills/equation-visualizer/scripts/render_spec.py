#!/usr/bin/env python3
"""Deterministic renderer: equation-explainer spec JSON -> standalone HTML.

The model never draws the final visual. It emits a spec (see
../../equation-understanding/spec.schema.json); this script turns that spec into a
...
Usage:
    python render_spec.py specs/<file>.json -o output/demo.html
"""
import argparse
import html
import json
...
def build_legend(spans, callouts_by_span):
    cards = []
    for s in spans:
        cards.append(
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .pane { background:var(--card); border:1px solid var(--border); border-radius:10px; padding:16px; }
  .pane h2 { margin:0 0 10px; font-size:12px; text-transform:uppercase; letter-spacing:.06em; color:var(--muted); }
  #left .summary { font-size:13.5px; line-height:1.55; color:#374151; }
  #left .srcbox { margin-top:12px; background:#0f172a; color:#e2e8f0; border-radius:8px; padding:10px 12px;
                  font-family:ui-monospace, Consolas, monospace; font-size:11.5px; white-space:pre-wrap; word-break:break-word; }
  #left .loc { margin-top:8px; font-size:12px; color:var(--muted); }
  #center { position:relative; }
  #eqname { font-size:15px; font-weight:600; margin-bottom:6px; }
  #equation { padding:28px 8px; overflow-x:auto; }
  #center .hint { font-size:12px; color:var(--muted); text-align:center; }
  ...
  #fallback { display:none; margin:8px 0 0; padding:8px 12px; background:#fef2f2; border:1px solid #fecaca; border-radius:8px; font-size:12.5px; color:#991b1b; }
...
<main>
  <section class="pane" id="left">
    ...
    <div id="fallback">MathJax failed to load (offline?). The raw LaTeX on the left is the source of truth.</div>
  </section>
  <section class="pane" id="center">
    <h2>Equation</h2>
    <div id="eqname">__EQ_NAME__</div>
    <div id="equation">\\[__WRAPPED_LATEX__\\]</div>
    ...
    <svg id="callout-svg"><line x1="0" y1="0" x2="0" y2="0" stroke="#111827" stroke-width="1.5" stroke-dasharray="4 3"/></svg>
    <div id="callout"></div>
  </section>
...
<section id="bottom">
  <div class="pane">
    ...
    <div id="conf">
      <div id="confbar"><div style="width:__CONF_PCT__%"></div></div>
...
(function () {
  ...
  var connector = document.getElementById('callout-svg');
  ...
  function showCallout(id) {
    ...
    if (!target || !CALLOUTS[id]) { hideCallout(); return; }
  ...
  function hideCallout() { callout.style.display = 'none'; connector.style.display = 'none'; }
  ...
  function init() {
    ...
    document.body.addEventListener('click', function () {
      if (pinned) { setHl(pinned, false); hideCallout(); pinned = null; }
    });
  }
...
def main() -> int:
    ...
    for cb in spec.get("callout_boxes", []):
        if cb["anchor_span"] not in known:
            warnings.append(f"callout anchors unknown span '{cb['anchor_span']}'")
    for q in spec.get("source_quotes", []):
        for sid in q.get("supports", []):
            if sid not in known:
                warnings.append(f"quote supports unknown span '{sid}'")

    wrapped = wrap_spans(eq["equation_latex"], spans, warnings.append)
    callouts = {cb["anchor_span"]: cb["text"] for cb in spec.get("callout_boxes", [])}
    ...
    page = (TEMPLATE
        ...
        .replace("__SUMMARY__", esc(paper.get("summary", "")))
        ...
        .replace("__SPEC_JSON__", json.dumps(spec, ensure_ascii=False).replace("</", "<\\/"))
    )
